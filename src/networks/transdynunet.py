...
from src.networks.modules import Transformer

__all__ = ["TransDynUNet", "TransDynUnet", "TransDynunet"]
...
class TransDynUNet(nn.Module):
    ...
    def __init__(
        ...
        bottle_neck_size: Sequence[Union[Sequence[int], int]],
        config = Optional,
        ...
        super(TransDynUNet, self).__init__()
        ...
        self.bottleneck = self.get_bottleneck(config)   # TODO: replace with transformer
    ...
    def get_bottleneck(self, config):
        ...
        return nn.Sequential(
            Transformer(
                config,
                self.filters[-1],
                tuple(np.int8(self.bottle_neck_size*np.array(self.strides[-1]))),
                self.device
            ),
            self.conv_block(
                self.spatial_dims,
...
TransDynUnet = TransDynunet = TransDynUNet
